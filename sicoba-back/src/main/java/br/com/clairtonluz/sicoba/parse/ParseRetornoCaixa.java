...
public class ParseRetornoCaixa {
    ...
    private Header lerArquivoRetornoCaixa(InputStream input, String fileName) throws IOException {
        ...
        while ((line = in.readLine()) != null) {
            int tipo = StringUtils.getInt(line, 7, 8);
            switch (tipo) {
                case HEADER:
    ...
    private Header preencheHeader(String line, String fileName) {
        ...
        h.setDataGeracao(StringUtils.getDataHora(line, 143, 157));
        h.setSequencial(StringUtils.getInt(line, 157, 163));
    ...
    private HeaderLote preencheHeaderLote(String line, Header header) {
        ...
        hl.setHeader(header);
        hl.setNumeroRemessaRetorno(StringUtils.getInt(line, 183, 191));
        hl.setDataGravacaoRemessaRetorno(StringUtils.getData(line, 191, 199));
    ...
    private Registro preencheRegistro(String line, HeaderLote headerLote, BufferedReader in)
        ...
        r.setCodigoMovimento(StringUtils.getInt(line, 15, 17));
        if (r.getCodigoMovimento() != Registro.TARIFA) {
            r.setModalidadeNossoNumero(StringUtils.getInt(line, 39, 41));
            r.setNossoNumero(StringUtils.getInt(line, 41, 56));
            r.setNumeroDocumento(StringUtils.get(line, 58, 69).trim());
            r.setVencimento(StringUtils.getData(line, 73, 81));

        }
        r.setHeaderLote(headerLote);
        r.setValorTitulo(StringUtils.getDouble2Decimal(line, 81, 96));
        r.setBancoRecebedor(StringUtils.get(line, 96, 99));
        r.setAgenciaRecebedor(StringUtils.get(line, 99, 104));
        r.setDigitoVerificadorRecebedor(StringUtils.get(line, 104, 105));
        r.setUsoDaEmpresa(StringUtils.get(line, 105, 130));
        r.setValorTarifa(StringUtils.getDouble2Decimal(line, 198, 213));
        ...
        if ((line = in.readLine()) != null) {
            ...
            rd.setRegistro(r);
            rd.setJurosMultasEncargos(StringUtils.getDouble2Decimal(line, 17, 32));
            rd.setDesconto(StringUtils.getDouble2Decimal(line, 32, 47));
            rd.setAbatimento(StringUtils.getDouble2Decimal(line, 47, 62));
            rd.setIof(StringUtils.getDouble2Decimal(line, 62, 77));
            rd.setValorPago(StringUtils.getDouble2Decimal(line, 77, 92));
            rd.setValorLiquido(StringUtils.getDouble2Decimal(line, 92, 107));
            rd.setDataOcorrencia(StringUtils.getData(line, 137, 145));
            String dataCredito = StringUtils.get(line, 145, 153);
            if (!dataCredito.equals("00000000")) {
                rd.setDataCredito(StringUtils.getData(line, 145, 153));
            }

            if (r.getCodigoMovimento() == Registro.LIQUIDACAO) {
                rd.setDataDebitoTarifa(StringUtils.getData(line, 157, 165));
            }
    ...
    private TrailerLote preencheTrailerLote(String line, HeaderLote headerLote) {
        ...
        tl.setQuantidadeRegistroLote(StringUtils.getInt(line, 17, 23));
        return tl;
    }
    ...
    private Trailer preencheTrailer(String line, Header header) {
        ...
        t.setHeader(header);
        t.setQuantidadeLotes(StringUtils.getInt(line, 17, 23));
        t.setQuantidadeRegistros(StringUtils.getInt(line, 23, 29));
        return t;
    }
}
