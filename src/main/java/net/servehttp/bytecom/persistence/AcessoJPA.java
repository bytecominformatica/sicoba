...
public class AcessoJPA {
	...
	public Acesso buscaPorIp(String ip) {
		try {
			return em
					.createQuery("select a from Acesso a where a.ip = :ip",
							Acesso.class).setParameter("ip", ip)
					.getSingleResult();
	...
	public Acesso buscaPorMac(String mac) {
		try {
			return em
					.createQuery("select a from Acesso a where a.mac = :mac",
							Acesso.class).setParameter("mac", mac)
					.getSingleResult();
		} catch (javax.persistence.NoResultException e) {
			return null;
		}
