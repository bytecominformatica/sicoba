...
import net.servehttp.bytecom.service.financeiro.ParseArquivoRetornoCaixa;
...
@Named
@RequestScoped
public class RetornoController extends GenericoController {
    private static final long serialVersionUID = -3249445210310419657L;

    private Part file;
    @Inject
    private ParseArquivoRetornoCaixa parseArquivoRetornoCaixa;
    @Inject
        ...
        if (file != null) {
            ...
            try {
                if (isArquivoRetorno(file)) {
                    header = parseArquivoRetornoCaixa.parse(file.getInputStream(), file.getSubmittedFileName());
                }
                if (notExists(header)) {
