...
MYSQLDRIVER=/tmp/mysql-connector-java-5.1.37.jar;
if ! [ -f $MYSQLDRIVER ]; then

	echo "DOWNLOADING MYSQL DRIVER 5.1.37";
	wget http://central.maven.org/maven2/mysql/mysql-connector-java/5.1.37/mysql-connector-java-5.1.37.jar;
	
fi;

$WILDFLY_DIR/bin/./jboss-cli.sh --connect --command='module add --name=com.mysql --resources=/tmp/mysql-connector-java-5.1.37.jar --dependencies=javax.api' 
